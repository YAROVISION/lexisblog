...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    /* Futuristic Hologram Loader Overlay */
    .loader-overlay {{
      position: fixed;
      top: 0;
      left: 0;
      width: 100vw;
      height: 100vh;
      background: radial-gradient(ellipse 85% 85% at 50% 50%, rgba(20, 17, 15, 0.94) 0%, rgba(8, 6, 5, 0.98) 100%);
      ...
      opacity: 1;
      visibility: visible;
      transition: opacity 0.7s cubic-bezier(0.16, 1, 0.3, 1), transform 0.7s cubic-bezier(0.16, 1, 0.3, 1), filter 0.7s ease, visibility 0.7s ease;
      pointer-events: all;
    }}
    ...
    .loader-card {{
      ...
      max-width: 420px;
      padding: 34px 28px;
      background: rgba(36, 33, 30, 0.85);
      border: 1px solid rgba(255, 255, 255, 0.12);
      border-radius: 20px;
      box-shadow: 0 24px 60px rgba(0, 0, 0, 0.8), 0 0 50px rgba(26, 107, 255, 0.18);
      position: relative;
      overflow: hidden;
    }}
    .loader-card::before {{
      ...
      width: 200%;
      height: 200%;
      background: radial-gradient(circle, rgba(0, 194, 168, 0.14) 0%, transparent 60%);
      animation: ambientGlow 7s ease-in-out infinite alternate;
      pointer-events: none;
    }}
    @keyframes ambientGlow {{
      0% {{ transform: translate(0, 0) scale(1); }}
      100% {{ transform: translate(8%, 8%) scale(1.25); }}
    }}
    .loader-scanner {{
      position: relative;
      width: 90px;
    ...
    .loader-ring-1 {{
      ...
      border-bottom-color: rgba(0, 194, 168, 0.2);
    ...
    .loader-ring-2 {{
      ...
      border-right-color: rgba(26, 107, 255, 0.2);
      animation: spinRingRev 2.0s linear infinite;
    }}
    .loader-ring-3 {{
      width: 52px;
      height: 52px;
      border-top-color: #EFD146;
      border-left-color: rgba(239, 209, 70, 0.25);
    ...
    .loader-core-icon {{
      ...
      justify-content: center;
      color: #fff;
      filter: drop-shadow(0 0 10px rgba(26, 107, 255, 0.8));
    ...
    .loader-title {{
      ...
      background: linear-gradient(135deg, #ffffff 0%, #e2ded9 50%, #00C2A8 100%);
    ...
    .loader-progress-bar {{
      height: 100%;
      width: 0%;
      background: linear-gradient(90deg, #1A6BFF, #00C2A8, #EFD146);
      border-radius: 999px;
      box-shadow: 0 0 12px rgba(0, 194, 168, 0.85);
...
<body>
  ...
  <!-- Hologram Cosmic Loader Overlay -->
      ...
      <div class="loader-scanner">
        ...
        <div class="loader-core-icon">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
            <circle cx="12" cy="12" r="3.5" stroke="#fff" fill="rgba(26,107,255,0.3)"/>
            <path d="M12 2v3M12 19v3M2 12h3M19 12h3" stroke="currentColor" stroke-linecap="round"/>
            <circle cx="12" cy="2" r="1.5" fill="#00C2A8" stroke="none"/>
            <circle cx="22" cy="12" r="1.5" fill="#1A6BFF" stroke="none"/>
            <circle cx="12" cy="22" r="1.5" fill="#EFD146" stroke="none"/>
            <circle cx="2" cy="12" r="1.5" fill="#FF5A5F" stroke="none"/>
          </svg>
        </div>
      </div>
      <div class="loader-title">Lexis Graphify</div>
      <div class="loader-subtitle">ГЛИБОКИЙ ГРАФ ЗНАНЬ</div>
      <div class="loader-status-text" id="loader-status-text">Ініціалізація нейромережі знань...</div>
      <div class="loader-progress-track">
        <div class="loader-progress-bar" id="loader-progress-bar"></div>
      </div>
      <div class="loader-meta">
        <span class="highlight">{len(export_nodes)}</span> ВУЗЛІВ
        <span>•</span>
        <span class="highlight">{len(edges)}</span> ЗВ'ЯЗКІВ
        <span>•</span>
        <span class="highlight">4</span> КЛАСТЕРИ
      </div>
    </div>
  </div>
    ...
    (function() {{
      ...
      window.addEventListener('message', (e) => {{
        ...
        if (e.data.type === 'ACTIVATE' || e.data.type === 'RESIZE') {{
          handleResize();
          if (e.data.isFirstLoad || !hasStartedIntro) {{
            startIntroAnimation();
          }}
      ...
      function startIntroAnimation(showOverlay = true) {{
        hasStartedIntro = true;
        isIntroAnimating = true;
        animStartTime = performance.now();

        // Position nodes initially near center singularity with tiny initial spread
        for (let i = 0; i < nodes.length; i++) {{
          const n = nodes[i];
          n.x = (n.targetX || 0) * 0.03;
          n.y = (n.targetY || 0) * 0.03;
          n.animProgress = 0;
        }}
        ...
        // Reset and trigger cosmic shockwaves
        shockwaves = [
          {{
            startTime: performance.now(),
            duration: 1600,
            maxRadius: Math.hypot(width, height) * 0.75,
            color: '#00C2A8',
            opacity: 0.85
          }},
          {{
            startTime: performance.now() + 180,
            duration: 1800,
            maxRadius: Math.hypot(width, height) * 0.65,
            color: '#1A6BFF',
            opacity: 0.65
          }}
        ];
        ...
        // Fallback timer to guarantee overlay disappears
        setTimeout(() => {{
          if (loaderOverlay) {{
            loaderOverlay.classList.add('hidden');
          }}
        ...
        function updateLoader(now) {{
          ...
          if (statusText) {{
            if (percent < 28) {{
              statusText.textContent = "Ініціалізація нейромережі знань...";
            }} else if (percent < 60) {{
              statusText.textContent = "Завантаження " + nodes.length + " концептів та правил...";
            }} else if (percent < 88) {{
              statusText.textContent = "Побудова " + links.length + " семантичних зв’язків...";
            }} else {{
              statusText.textContent = "Синхронізація просторового графа...";
            }}
          }}
      ...
      function render() {{
        ...
        if (isIntroAnimating) {{
          ...
          for (let i = 0; i < nodes.length; i++) {{
            ...
            const nodeDelay = distRatio * 320;
            const t = Math.max(0, Math.min(1.0, (elapsed - nodeDelay) / 950));
            
            if (t < 1.0) allDone = false;
            
            const ease = t === 1.0 ? 1.0 : 1 - Math.pow(1 - t, 4);
            n.x = n.targetX * ease;
            n.y = n.targetY * ease;
            n.animProgress = ease;
          }}

          if (elapsed > 1600 && allDone) {{
        ...
        for (let s = 0; s < shockwaves.length; s++) {{
          ...
          if (elapsed > 0 && elapsed < sw.duration) {{
            const p = elapsed / sw.duration;
            const easeP = 1 - Math.pow(1 - p, 3);
            const r = sw.maxRadius * easeP;
            const alpha = sw.opacity * (1 - p);
      ...
      // Initialize entrance animation on load
