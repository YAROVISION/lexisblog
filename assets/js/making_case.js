/**
 * Making Your Case - Interactive Book Reader & Search
 * Pure Vanilla JS
 */

document.addEventListener('DOMContentLoaded', () => {
  const bookData = window.MAKING_CASE_DATA;
  ...
  function initSectionFilters() {
    ...
    // 'All' button already in HTML or dynamically rendered
    ...
    bookData.sections.forEach(sec => {
      ...
      // Shorten label for chips if needed
      let label = sec;
    ...
    sectionsToRender.forEach(sec => {
      ...
      secHeader.innerHTML = `
        <div class="toc-sec-title-wrap">
          <span class="toc-sec-badge">${segs.length} сегментів</span>
        ...
        item.innerHTML = `
          <div class="toc-item-left">
            ...
            <div class="seg-info">
              ...
              <span class="seg-meta">${seg.pages > 1 ? seg.pages + ' сторінки' : '1 сторінка'} • ${formatCharCount(seg.text.length)}</span>
            </div>
          </div>
  ...
  function performSearch() {
    ...
    if (q === '') {
      ...
      searchStats.textContent = `Всього 124 сегменти книги`;
      return;
    }
    ...
    results.forEach(seg => {
      ...
      card.innerHTML = `
        ...
        <div class="search-card-footer">
          <span class="seg-meta">${seg.pages} стор. • ${formatCharCount(seg.text.length)}</span>
  ...
  function openReader(segmentId, highlightTermsList = null) {
    ...
    modalSegmentMeta.innerHTML = `
      <span>Сегмент <strong>#${seg.id}</strong> із 124</span>
      <span>•</span>
      <span>${seg.pages > 1 ? seg.pages + ' сторінки' : '1 сторінка'}</span>
      <span>•</span>
    ...
    if (seg.subsegments && seg.subsegments.length > 1) {
      ...
      subContainer.innerHTML = `<h4 style="margin: 1.5rem 0 1rem; font-size: 1.1rem; color: var(--text-primary);">Підсегменти (${seg.subsegments.length}):</h4>`;
      ...
      seg.subsegments.forEach(sub => {
        ...
        subItem.innerHTML = `
          <div class="subsegment-badge">Підсегмент ${sub.id} • ${escapeHTML(sub.pages || '')}</div>
  ...
  if (modalCopyBtn) {
    modalCopyBtn.addEventListener('click', () => {
      ...
      const citation = `\n\n— Антонін Скалія, Браян А. Гарнер. «Making Your Case» (#${seg.id}: ${seg.title})`;
  ...
  // 6. Check URL Hash on Load (e.g. #segment-27)
  function checkHash() {
  ...
  function formatSegmentText(rawText) {
    if (!rawText) return '';
    // Split by newlines or sentence boundaries if long single block
    const cleaned = escapeHTML(rawText);
    const paragraphs = cleaned.split(/\n\s*\n|\n/).filter(p => p.trim() !== '');
    if (paragraphs.length <= 1) {
      // Split into sensible paragraphs of ~2-3 sentences if it's one huge block
  ...
  function highlightTerms(text, terms) {
    ...
    let result = text;
    // Sort terms longest first to avoid partial replacement overlaps
