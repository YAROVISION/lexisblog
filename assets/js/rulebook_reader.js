/**
 * Anthony Weston - A Rulebook for Arguments (5th Edition)
 * Interactive Reader & Full-Text Search
 * Pure Vanilla JS
 */

document.addEventListener('DOMContentLoaded', () => {
  const bookData = window.RULEBOOK_DATA;
  if (!bookData || !bookData.segments) {
    console.error('Rulebook data not loaded.');
    return;
  }
  ...
  function renderTOC() {
    ...
    sectionsToRender.forEach(sec => {
      ...
      segs.forEach(seg => {
        ...
        const pageLabel = seg.pages_label ? seg.pages_label : (seg.pages > 1 ? `${seg.pages} стор.` : '1 стор.');
        ...
        item.innerHTML = `
          <div class="toc-item-left">
            ...
            <div class="seg-info">
              ...
              <span class="seg-meta">${escapeHTML(pageLabel)} • ${formatCharCount(seg.text.length)}</span>
            </div>
          </div>
  ...
  function performSearch() {
    ...
    if (q === '') {
      ...
      searchStats.textContent = `Всього ${bookData.segments.length} сегментів праці (50 правил, додатки та довідковий апарат)`;
      return;
    }
  ...
  function renderSearchResults(results, terms) {
      ...
      searchResultsContainer.innerHTML = `
        <div class="empty-search-state bento-card">
          ...
          <p style="color: var(--text-secondary); margin-bottom: 1.5rem;">
            Спробуйте перевірити правопис або вибрати фільтр «Усі розділи».
          </p>
    ...
    results.forEach(seg => {
      ...
      const pageLabel = seg.pages_label ? seg.pages_label : `${seg.pages} стор.`;
