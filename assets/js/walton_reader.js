 ...
 * Interactive Reader & Search
 * Pure Vanilla JS
 */

document.addEventListener('DOMContentLoaded', () => {
  const bookData = window.WALTON_DATA;
  ...
  function renderTOC() {
    ...
    sectionsToRender.forEach(sec => {
      ...
      segs.forEach(seg => {
        ...
        const pageLabel = seg.pages_label ? `с. ${seg.pages_label}` : (seg.pages > 1 ? `${seg.pages} стор.` : '1 стор.');
  ...
  function openReader(segmentId, highlightTermsList = null) {
    ...
    const pageLabel = seg.pages_label ? `Сторінки: ${seg.pages_label}` : `${seg.pages} стор.`;
    if (modalSegmentMeta) {
